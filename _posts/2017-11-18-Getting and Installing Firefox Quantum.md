...
Anyway, hope this helps satisfy ones craving for the latest browser tech. If you want this to only to be available to a single user - that's easy! Just set `dir=$HOME` in step #1, `NEW_DESKD="$HOME/.local/share/applications"` in step #2, and drop `sudo` privileges for both commands if this is for yourself.
